...
typedef enum {_CHAN_k_MID_flow=0} _CHAN_k_MID_t;
...
static upper_bound_t tinv[N_PLACES] =
  {{-1,0}, {-1,0}, {5,90}, {1,0}, {4,10250}, {-1,0}, {3,300}
  };
...
fprint_label(FILE *f, label_t *lab) {
  ...
  if (lab->clock >= 0) {
    fprintf(f,"H%d >= %d => ",lab->clock,lab->bound);
  }
  else {
    fprintf(f,"true => ");
  }
  ...
  fprintf(f,"H1");
  for (i = 0; lab->resets[i] != -1; i++) {
    if (lab->resets[i] != 1) {
      fprintf(f," H%d",lab->resets[i]);
    }
  }
  fprintf(f," }; ");
}
...
/* h0 is reserved now -- so don't print it */
  int i;
  if (N_CLOCKS > 0) {
    fprintf(f,"H1");
    for (i = 2; i < N_CLOCKS; i++) {
     fprintf(f," H%d",i);
    }
  }
}
...
start_state(state_vector_t *svec) {
  uchar init_marking[MARKING_N_BYTES] = {0x2c};
...
iterate_state(state_vector_t *s1, label_t *lab, 
  ...
  if (marking != 0) {
    ...
    if (marking & 0x10) { /* trigger 3 */
        ...
        q_insert(s2,_CHAN_k,_CHAN_k_MID_flow,s1->env._VAR_x);
        lab->type = SND; lab->k = _CHAN_k;
        lab->i = _CHAN_k_MID_flow; lab->v = s1->env._VAR_x;
    ...
    if (marking & 0x08) { /* trigger 4 */
        char vw[MARKING_N_BYTES] = {0x78};
        char t[MARKING_N_BYTES] = {0x28};
        memcpy((char *)s2, (char *)s1, size_state()); 
        for (i = 0; i < MARKING_N_BYTES; i++) {
          s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
        }
        lab->type = COMP; strcpy(lab->name,"OP_PERIOD");
        lab->clock = 4; lab->bound = 10000;
        lab->resets[0] = 4;
        lab->resets[1] = 5;
        lab->resets[2] = -1;
    ...
    if (marking & 0x04) { /* trigger 5 */
      if ((s1->sidv[_CHAN_k].status == ACCPT) && (s1->sidv[_CHAN_k].id == _CHAN_k_MID_flow)) {
        char vw[MARKING_N_BYTES] = {0x04};
        char t[MARKING_N_BYTES] = {0x02};
        memcpy((char *)s2, (char *)s1, size_state()); 
        for (i = 0; i < MARKING_N_BYTES; i++) {
          s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
        }
        ...
        lab->i = _CHAN_k_MID_flow; lab->v = s1->sidv[_CHAN_k].value;
        lab->clock = -1; lab->bound = 0;
        lab->resets[0] = 3;
        lab->resets[1] = -1;
        stopped[_CHAN_k] = 1;
...
fprint_invariant_one_constraint(FILE *f, int clock, int value) {
  if (value > -1) {  /* -1 represents +infinity */ 
    if (first_conjunct) {
      first_conjunct = 0;
    }
    else {
  ...
  for (i = 0; i < MARKING_N_BYTES; i++) {
    if (svec->marking[i]) {
      ...
      if ((svec->marking[i] & 0x80) && (PRED[p](&svec->env))) {
        fprint_invariant_one_constraint(f,tinv[p].clock,tinv[p].bound);
      }
      p += 1;
