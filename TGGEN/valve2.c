...
#define N_CLOCKS 7
static short clock_max_bound[N_CLOCKS] = {0, 0, 4, 6, 4, 9, 2};
/********************** DBMS **************************************/
/******************** Bounds ********************************************/
#include <values.h>
...
start_state(state_vector_t *svec) {
  uchar init_marking[MARKING_N_BYTES] = {0x2d, 0x00};
...
iterate_state(state_vector_t *s1, label_t *lab, 
  ...
  if (marking != 0) {
    ...
    if (marking & 0x20) { /* trigger 2 */
        ...
        intersect_lower_bound_matrix(s1_matrix, 6, 1);
    ...
    if (marking & 0x08) { /* trigger 4 */
        ...
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 1, 1};
            memcpy(reset_clock,r,N_CLOCKS);
          }
    ...
    if (marking & 0x04) { /* trigger 5 */
      if ((s1->sidv[_CHAN_k].status == ACCPT) && (s1->sidv[_CHAN_k].id == _CHAN_k_MID_pressure)) {
          ...
          char t[MARKING_N_BYTES] = {0x02, 0x00};
          memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          s2->env._VAR_y1 = s1->sidv[_CHAN_k].value;
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 1, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(s1_matrix,new_matrix);
    ...
    if (marking & 0x02) { /* trigger 6 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 4, 3);
        if (canonicalize_matrix(s1_matrix)) {
          char vw[MARKING_N_BYTES] = {0x02, 0x00};
          char t[MARKING_N_BYTES] = {0x00, 0x00};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          ...
          lab->type = COMP; strcpy(lab->name,"OP_AdjustValve1");
          OP_AdjustValve1(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            set_active_clocks(s2);
            init_succ_matrix(s1_matrix,new_matrix);
    ...
    if (marking & 0x01) { /* trigger 7 */
      if ((s1->sidv[_CHAN_k].status == ACCPT) && (s1->sidv[_CHAN_k].id == _CHAN_k_MID_pressure)) {
          char vw[MARKING_N_BYTES] = {0x01, 0x00};
          char t[MARKING_N_BYTES] = {0x00, 0x80};
          memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          s2->env._VAR_y2 = s1->sidv[_CHAN_k].value;
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 1, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(s1_matrix,new_matrix);
  ...
  if (marking != 0) {
    if (marking & 0x80) { /* trigger 8 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 3, 5);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          lab->type = COMP; strcpy(lab->name,"OP_AdjustValve2");
          OP_AdjustValve2(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            set_active_clocks(s2);
            init_succ_matrix(s1_matrix,new_matrix);
