...
#define N_CLOCKS 4
static short clock_max_bound[N_CLOCKS] = {0, 0, 1, 1};
/********************** DBMS **************************************/
/******************** Bounds ********************************************/
#include <values.h>
...
static short n_mids_per_chan[N_CHANNELS] = {};

static short q_max_n[N_CHANNELS] = {};

static short q_offset[N_CHANNELS] = {};

static int net_clocks[N_CHANNELS] = {};
...
start_state(state_vector_t *svec) {
  uchar init_marking[MARKING_N_BYTES] = {0x20};
...
iterate_state(state_vector_t *s1, label_t *lab, 
  ...
  if (marking != 0) {
    ...
    if (marking & 0x10) { /* trigger 3 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 2, 1);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          lab->type = COMP; strcpy(lab->name,"OP_incx");
          OP_incx(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            set_active_clocks(s2);
    ...
    if (marking & 0x04) { /* trigger 5 */
      if (PRED[5](&(s1->env))) { /* PRED_isxfive */
          char vw[MARKING_N_BYTES] = {0x04};
          char t[MARKING_N_BYTES] = {0x08};
          memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = GU; strcpy(lab->name,"PRED_isxfive");
          open_guard = 1;
          doit(s1,lab,s2);
        }
    }
