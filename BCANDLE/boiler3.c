...
#define N_CLOCKS 7
static short clock_max_bound[N_CLOCKS] = {0, 0, 5, 10, 2, 20, 4};
/********************** DBMS **************************************/
/******************** Bounds ********************************************/
#include <values.h>
...
fprint_active_clocks(FILE *f) {
  ...
  for (i = 0; i < N_CLOCKS; i+=1) {
    if (active_clock[i]) {
      printf("C%d is active\n",i);
    }
    else {
      printf("C%d is not active\n",i);
    }
  }
}
...
typedef enum {_CHAN_k_MID_temperature=0, _CHAN_k_MID_sensor_failure=1, _CHAN_k_MID_temp_high=2, _CHAN_k_MID_temp_low=3} _CHAN_k_MID_t;
...
{{{1, 4, 5, 1, 2}, {2, 4, 5, 1, 2}, {3, 4, 5, 1, 2}, {4, 4, 5, 1, 2}}};

static short n_mids_per_chan[N_CHANNELS] = {4};

static short q_max_n[N_CHANNELS] = {4};
...
set_active_clocks(state_vector_t *svec) {
  ...
  for (i = 0; i < MARKING_N_BYTES; i++) {
    if (svec->marking[i]) {
      p = i * 8;
      if ((svec->marking[i] & 0x80) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 1;
      if ((svec->marking[i] & 0x40) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 2;
      if ((svec->marking[i] & 0x20) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 3;
      if ((svec->marking[i] & 0x10) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 4;
      if ((svec->marking[i] & 0x08) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 5;
      if ((svec->marking[i] & 0x04) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 6;
      if ((svec->marking[i] & 0x02) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
      p = i * 8 + 7;
      if ((svec->marking[i] & 0x01) && (tinv[p].clock > -1)) {
        active_clock[tinv[p].clock] = 1;
      }
    }
  }
...
void
intersect_invariant_matrix(state_vector_t *svec) {
  int i,k,c,p;
  uchar C2I[N_CLOCKS];  /* Clock to Index */
  memset(C2I,0,N_CLOCKS);
  for (i = 1; i < matrix_dimension(new_matrix); i+=1) {
    C2I[AT(new_matrix,i,i)] = i;
  }  
  for (k = 0; k < N_CHANNELS; k++) {
    ...
    case FREE : {
      ...
      if ((*(uint *)q) != BLANK_MESSAGE) {
        c = C2I[net_clocks[k]];
        AT(new_matrix,c,0) = min_bound(AT(new_matrix,c,0),ZERO_LE);
      }
      break;
    }
    case PRE :
      c = C2I[net_clocks[k]];
      AT(new_matrix,c,0) = min_bound(AT(new_matrix,c,0),
  ...
  for (i = 0; i < MARKING_N_BYTES; i++) {
    if (svec->marking[i]) {
      ...
      if ((svec->marking[i] & 0x80) && (tinv[p].clock > -1)) {
        c = C2I[tinv[p].clock];
        AT(new_matrix,c,0) = min_bound(AT(new_matrix,c,0),tinv[p].bound);
      }
      p = i * 8 + 1;
      if ((svec->marking[i] & 0x40) && (tinv[p].clock > -1)) {
        c = C2I[tinv[p].clock];
...
start_state(state_vector_t *svec) {
  uchar init_marking[MARKING_N_BYTES] = {0x2c, 0x00};
  ...
  intersect_invariant_matrix(svec);
...
iterate_state(state_vector_t *s1, label_t *lab, 
  ...
  if (marking != 0) {
    ...
    if (marking & 0x20) { /* trigger 2 */
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix, 6, 2);
        if (canonicalize_matrix(temp_matrix)) {
          char vw[MARKING_N_BYTES] = {0x70, 0x00};
          char t[MARKING_N_BYTES] = {0x10, 0x00};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 0, 0};
...
#endif 
          lab->type = COMP; strcpy(lab->name,"OP_ReadTemperature");
          OP_ReadTemperature(&(s1->env),data);
    ...
    if (marking & 0x10) { /* trigger 3 */
          char vw[MARKING_N_BYTES] = {0x10, 0x00};
          char t[MARKING_N_BYTES] = {0x40, 0x00};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          {
            uchar r[N_CLOCKS] = {1, 0, 0, 0, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          q_insert(s2,_CHAN_k,_CHAN_k_MID_temperature,s1->env._VAR_v1);
          ...
          lab->i = _CHAN_k_MID_temperature; lab->v = s1->env._VAR_v1;
          doit(s1,lab,s2);
    }
    if (marking & 0x08) { /* trigger 4 */
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix, 5, 19);
        if (canonicalize_matrix(temp_matrix)) {
          char vw[MARKING_N_BYTES] = {0x78, 0x00};
          char t[MARKING_N_BYTES] = {0x28, 0x00};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          {
            uchar r[N_CLOCKS] = {1, 0, 0, 0, 0, 1, 1};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(temp_matrix,new_matrix);
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = COMP; strcpy(lab->name,"OP_Period");
          OP_Period(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            doit(s1,lab,s2);
          }
    ...
    if (marking & 0x04) { /* trigger 5 */
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix, 4, 0);
        if (canonicalize_matrix(temp_matrix)) {
          char vw[MARKING_N_BYTES] = {0x04, 0x00};
          char t[MARKING_N_BYTES] = {0x02, 0x04};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          {
            uchar r[N_CLOCKS] = {1, 0, 0, 1, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(temp_matrix,new_matrix);
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = COMP; strcpy(lab->name,"OP_Configure");
          OP_Configure(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            doit(s1,lab,s2);
          }
    ...
    if (marking & 0x02) { /* trigger 6 */
      if ((s1->sidv[_CHAN_k].status == ACCPT) && (s1->sidv[_CHAN_k].id == _CHAN_k_MID_temperature)) {
          char vw[MARKING_N_BYTES] = {0x02, 0x06};
          char t[MARKING_N_BYTES] = {0x00, 0xa8};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          lab->i = _CHAN_k_MID_temperature; lab->v = s1->sidv[_CHAN_k].value;
          stopped[_CHAN_k] = 1;
          doit(s1,lab,s2);
      }
    }
    if (marking & 0x01) { /* trigger 7 */
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix, 1, 0);
        if (canonicalize_matrix(temp_matrix)) {
          char vw[MARKING_N_BYTES] = {0x01, 0x7e};
          char t[MARKING_N_BYTES] = {0x04, 0x00};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
          {
            uchar r[N_CLOCKS] = {1, 0, 0, 0, 1, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(temp_matrix,new_matrix);
  ...
  if (marking != 0) {
    if (marking & 0x80) { /* trigger 8 */
      if (PRED_TempOk(&(s1->env))) {
          char vw[MARKING_N_BYTES] = {0x00, 0x80};
          char t[MARKING_N_BYTES] = {0x01, 0x00};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = GU; strcpy(lab->name,"PRED_TempOk");
          open_guard = 1;
          doit(s1,lab,s2);
        }
    }
    if (marking & 0x40) { /* trigger 9 */
          char vw[MARKING_N_BYTES] = {0x01, 0xde};
          char t[MARKING_N_BYTES] = {0x04, 0x00};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
          ...
          q_insert(s2,_CHAN_k,_CHAN_k_MID_temp_high,s1->env._VAR__);
          if (s1->sidv[_CHAN_k].status == FREE) {
            reset_clock[net_clocks[_CHAN_k]] = 1;
          }
          set_active_clocks(s2);
...
#endif 
          lab->type = SND; lab->k = _CHAN_k;
          lab->i = _CHAN_k_MID_temp_high; lab->v = s1->env._VAR__;
          doit(s1,lab,s2);
    }
    if (marking & 0x20) { /* trigger 10 */
      if (PRED_TempHigh(&(s1->env))) {
          char vw[MARKING_N_BYTES] = {0x00, 0x20};
          char t[MARKING_N_BYTES] = {0x00, 0x40};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = GU; strcpy(lab->name,"PRED_TempHigh");
          open_guard = 1;
          doit(s1,lab,s2);
        }
    }
    if (marking & 0x10) { /* trigger 11 */
          char vw[MARKING_N_BYTES] = {0x01, 0xf6};
          char t[MARKING_N_BYTES] = {0x04, 0x00};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
          ...
          q_insert(s2,_CHAN_k,_CHAN_k_MID_temp_low,s1->env._VAR__);
          if (s1->sidv[_CHAN_k].status == FREE) {
            reset_clock[net_clocks[_CHAN_k]] = 1;
          }
          set_active_clocks(s2);
...
#endif 
          lab->type = SND; lab->k = _CHAN_k;
          lab->i = _CHAN_k_MID_temp_low; lab->v = s1->env._VAR__;
          doit(s1,lab,s2);
    }
    if (marking & 0x08) { /* trigger 12 */
      if (PRED_TempLow(&(s1->env))) {
          char vw[MARKING_N_BYTES] = {0x00, 0x08};
          char t[MARKING_N_BYTES] = {0x00, 0x10};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          ...
          memcpy(s2->marx, new_matrix, MATRIX_N_BYTES);
#endif 
          lab->type = GU; strcpy(lab->name,"PRED_TempLow");
          open_guard = 1;
          doit(s1,lab,s2);
        }
    }
    if (marking & 0x04) { /* trigger 13 */
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix, 3, 9);
        if (canonicalize_matrix(temp_matrix)) {
          char vw[MARKING_N_BYTES] = {0x03, 0xfc};
          char t[MARKING_N_BYTES] = {0x00, 0x02};
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
            s2->marking[i] = (s1->marking[i] & ~vw[i]) | t[i];
          }
    ...
    if (marking & 0x02) { /* trigger 14 */
          char vw[MARKING_N_BYTES] = {0x03, 0xfa};
          char t[MARKING_N_BYTES] = {0x04, 0x00};
          memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
          memcpy((char *)s2, (char *)s1, size_state()); 
          for (i = 0; i < MARKING_N_BYTES; i++) {
          ...
          q_insert(s2,_CHAN_k,_CHAN_k_MID_sensor_failure,s1->env._VAR__);
          if (s1->sidv[_CHAN_k].status == FREE) {
            reset_clock[net_clocks[_CHAN_k]] = 1;
          }
          set_active_clocks(s2);
...
#endif 
          lab->type = SND; lab->k = _CHAN_k;
          lab->i = _CHAN_k_MID_sensor_failure; lab->v = s1->env._VAR__;
          doit(s1,lab,s2);
    }
  }
  ...
  if (!open_guard) {/* network transitions */
    for (k = 0; k < N_CHANNELS; k++) {
      ...
      case PRE :
        memcpy(temp_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(temp_matrix,net_clocks[k],dlb(k,s1->sidv[k].id));
        if (canonicalize_matrix(temp_matrix)) {
          memcpy((char *)s2, (char *)s1, size_state()); 
...
#else
  data = new_stack(sizeof(data_env_t));
  q = new_queue(sizeof(state_vector_t));
#ifdef MA
  dfa_init(size_state());
#else
