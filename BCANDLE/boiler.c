...
#define N_CLOCKS 9
static short clock_max_bound[N_CLOCKS] = {0, 0, 53, 0, 105, 0, 11, 205, 32};
/********************** DBMS **************************************/
/******************** Bounds ********************************************/
#include <values.h>
...
typedef enum {_CHAN_k_MID_temperature=0} _CHAN_k_MID_t;
...
static upper_bound_t tinv[N_PLACES] = {
  {-1,0},
  {-1,0},
  {8, mk_bound(32, SGN_LE)},
  {1, ZERO_LE},
  {7, mk_bound(205, SGN_LE)},
  {6, mk_bound(11, SGN_LE)},
  {-1,0},
  {1, ZERO_LE},
  {1, ZERO_LE},
  {5, INFTY_LT},
  {1, ZERO_LE},
  {4, mk_bound(105, SGN_LE)},
  {3, INFTY_LT}
};
...
iterate_state(state_vector_t *s1, label_t *lab, 
  ...
  if (marking != 0) {
    ...
    if (marking & 0x20) { /* trigger 2 */
        ...
        intersect_lower_bound_matrix(s1_matrix, 8, 24);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 0, 0, 0, 0};
    ...
    if (marking & 0x08) { /* trigger 4 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 7, 195);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 0, 0, 1, 1};
            memcpy(reset_clock,r,N_CLOCKS);
          }
    ...
    if (marking & 0x04) { /* trigger 5 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 6, 9);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 1, 0, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
    ...
    if (marking & 0x01) { /* trigger 7 */
        ...
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 0, 1, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
  ...
  if (marking != 0) {
    ...
    if (marking & 0x40) { /* trigger 9 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 5, 0);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          lab->type = COMP; strcpy(lab->name,"OP_RecoverControl");
          OP_RecoverControl(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            set_active_clocks(s2);
            init_succ_matrix(s1_matrix,new_matrix);
    ...
    if (marking & 0x20) { /* trigger 10 */
      if (PRED[10](&(s1->env))) { /* PRED_TempNok */
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 0, 0, 1, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
          set_active_clocks(s2);
          init_succ_matrix(s1_matrix,new_matrix);
    ...
    if (marking & 0x10) { /* trigger 11 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 4, 95);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          {
            uchar r[N_CLOCKS] = {1, 1, 0, 1, 0, 0, 0, 0, 0};
            memcpy(reset_clock,r,N_CLOCKS);
          }
    ...
    if (marking & 0x08) { /* trigger 12 */
        memcpy(s1_matrix,s1->marx,size_matrix(s1->marx));
        intersect_lower_bound_matrix(s1_matrix, 3, 0);
        if (canonicalize_matrix(s1_matrix)) {
          ...
          lab->type = COMP; strcpy(lab->name,"OP_SensorFailure");
          OP_SensorFailure(&(s1->env),data);
          while (data->top->nxtElem) {
            pop(data,&(s2->env));
            set_active_clocks(s2);
            init_succ_matrix(s1_matrix,new_matrix);
